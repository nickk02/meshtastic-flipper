...
def write_sim_frames(path):
    """Emit frames the app replays when no radio is attached.

    Generated from the same source as the test vectors so the simulated path
    and the tested path cannot drift apart, and so no frame bytes are ever
    hand-written into the app.
    """
    lines = [
    ...
    for i, case in enumerate(CASES):
        c = build_case(case)
        body = ", ".join("0x%02x" % b for b in c["frame"])
        lines.append("/* %s */" % c["label"])
        lines.append("static const uint8_t SIM_FRAME_%d[] = {%s};" % (i, body))

    lines.append("")
    lines.append("static const SimFrame SIM_FRAMES[] = {")
    for i, case in enumerate(CASES):
        c = build_case(case)
        lines.append(
            "    {SIM_FRAME_%d, %d, 0x%02x, %d},"
            % (i, len(c["frame"]), c["chash"], c["psk_index"])
        )
    lines.append("};")
    lines.append("")
    lines.append("#define SIM_FRAME_COUNT %d" % len(CASES))
    lines.append("")
    lines.append("#endif")
...
def main():
    if "--sim-frames" in sys.argv:
        write_sim_frames(sys.argv[sys.argv.index("--sim-frames") + 1])
        return
