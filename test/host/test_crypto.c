...
TEST(test_decrypt_recovers_plaintext) {
    uint8_t out[256];
    mesh_crypto_xcrypt(
        VEC0_KEY, VEC0_PACKET_ID, VEC0_FROM_NODE, VEC0_CIPHERTEXT, VEC0_CIPHERTEXT_LEN, out);
...
TEST(test_decrypt_long_payload_spans_blocks) {
    ...
    uint8_t out[256];
    mesh_crypto_xcrypt(
        VEC2_KEY, VEC2_PACKET_ID, VEC2_FROM_NODE, VEC2_CIPHERTEXT, VEC2_CIPHERTEXT_LEN, out);
...
TEST(test_decrypt_with_second_psk) {
    uint8_t out[256];
    mesh_crypto_xcrypt(
        VEC3_KEY, VEC3_PACKET_ID, VEC3_FROM_NODE, VEC3_CIPHERTEXT, VEC3_CIPHERTEXT_LEN, out);
...
TEST(test_wrong_key_does_not_recover) {
    ...
    mesh_crypto_xcrypt(
        bad_key, VEC0_PACKET_ID, VEC0_FROM_NODE, VEC0_CIPHERTEXT, VEC0_CIPHERTEXT_LEN, out);
...
TEST(test_wrong_packet_id_does_not_recover) {
    uint8_t out[256];
    mesh_crypto_xcrypt(
        VEC0_KEY, VEC0_PACKET_ID + 1, VEC0_FROM_NODE, VEC0_CIPHERTEXT, VEC0_CIPHERTEXT_LEN, out);
    ASSERT_TRUE(memcmp(out, VEC0_PLAINTEXT, VEC0_PLAINTEXT_LEN) != 0);
}
...
TEST(test_wrong_source_node_does_not_recover) {
    uint8_t out[256];
    mesh_crypto_xcrypt(
        VEC0_KEY, VEC0_PACKET_ID, VEC0_FROM_NODE + 1, VEC0_CIPHERTEXT, VEC0_CIPHERTEXT_LEN, out);
    ASSERT_TRUE(memcmp(out, VEC0_PLAINTEXT, VEC0_PLAINTEXT_LEN) != 0);
}
...
TEST(test_xcrypt_is_its_own_inverse) {
    ...
    uint8_t twice[256];
    mesh_crypto_xcrypt(
        VEC0_KEY, VEC0_PACKET_ID, VEC0_FROM_NODE, VEC0_PLAINTEXT, VEC0_PLAINTEXT_LEN, once);
    /* Encrypting the plaintext must reproduce the generator's ciphertext. */
    ASSERT_EQ_MEM(once, VEC0_CIPHERTEXT, VEC0_CIPHERTEXT_LEN);

    mesh_crypto_xcrypt(VEC0_KEY, VEC0_PACKET_ID, VEC0_FROM_NODE, once, VEC0_PLAINTEXT_LEN, twice);
    ASSERT_EQ_MEM(twice, VEC0_PLAINTEXT, VEC0_PLAINTEXT_LEN);
}

TEST(test_zero_length_is_safe) {
    uint8_t out[4] = {0xAA, 0xAA, 0xAA, 0xAA};
    mesh_crypto_xcrypt(VEC0_KEY, VEC0_PACKET_ID, VEC0_FROM_NODE, VEC0_CIPHERTEXT, 0, out);
